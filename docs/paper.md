...
`cqcpack` (v0.1.0) provides an automated workflow to access CQC data in bulk from their publicly accessible Syndication API [@Das2026]. The Care Quality Commission (CQC) is an independent regulator of health and social care services in England. CQC ratings are widely recognised as the gold standard for assessing the quality of such provision and are routinely consulted by a broad range of stakeholders such as service users and their families, local authorities, commissioners and researchers when they need to determine the standard of a registered provider. In addition to publishing rating information and detailed reports, the CQC operates a publicly accessible Syndication API releasing corresponding information on registration status, regulated activities, and many other service characteristics. Such granular data, though publicly accessible, presents a significant technical barrier to those seeking to up to date CQC datasets, highlighting the need for a more streamlined solution.

The package addresses this by retrieving all location and provider IDs from list endpoints, fetching each ID's JSON data, and converting responses into analysis-ready data frames. The package eliminates these barriers, democratising access to regulatory data for researchers and policy analysts regardless of programming background.
